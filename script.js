...
const EMAILJS_PUBLIC_KEY = "kBCdtfbYXNi_VmCae";
const EMAILJS_SERVICE_ID = "service_nktbq7j";
const EMAILJS_TEMPLATE_ID = "template_q5csn5h";
...
/*
~1.1s branded intro — previously 3.2s felt too slow.
*/
...
const loaderSteps = [
    { progress: 0, text: "Loading portfolio..." },
    { progress: 35, text: "Preparing sections..." },
    { progress: 70, text: "Almost ready..." },
...
/* =====================================================
START LOADER ASAP (don't wait for heavy assets)
===================================================== */
...
const sectionIds = ["home", "about", "skills", "projects", "cv", "contact"];
...
const projectOrder = ["financial", "sales", "analysis"];

const projectsData = {

    financial: {
        title: "Dynamic Sales Report Dashboard in Excel",
        subtitle: "A fully interactive Excel sales dashboard that automatically updates the title, date range, KPIs and summary tables based on the selected reporting period.",
        type: "Excel Dashboard",
        tags: ["Excel", "Data Visualization"],
        description: "I built this dynamic Sales Report dashboard in Excel as part of my Data Analysis learning journey. The report title, date range, and all KPIs automatically update based on the Start Date and End Date inputs — making it fully interactive and reusable for any reporting period. The dashboard relies on formulas and structured tables linked to raw transactional data, so changing the date filter instantly refreshes every KPI and table.",
        tools: ["Excel", "Data Visualization"],
        highlights: [
            {
                title: "Dynamic Title & Date Range",
                text: "The report header updates automatically to reflect the selected Start Date and End Date."
            },
            {
                title: "Leads, Conversions & Sales KPIs",
                text: "Live KPIs including Total Leads, Meetings, Deals, Conversion Rates, Total Sales, and AOV."
            },
            {
                title: "Marketing Channel Performance",
                text: "Compare Landing Page, TikTok, Snapchat, and Cold Outreach by leads, deals, and revenue."
            },
            {
                title: "Salesman Performance",
                text: "Track meetings, deals, conversion rate, total sales, and AOV for each salesman."
            },
            {
                title: "Revenue Summary Tables",
                text: "Quick comparison of Total Sales by Marketing Channel and AOV by Salesman."
            }
        ],
        work: [
    ...
    sales: {
        ...
        subtitle: "Track sales performance across branches and salesmen with live KPIs, date filters, and visual branch comparison.",
        type: "Sales Analysis",
        tags: ["Excel", "Sales Analysis", "Data Visualization"],
        description: "I built this dynamic Sales Report in Excel to track and analyze sales performance across branches and salesmen. The report title and all KPIs update automatically based on the selected Start Date and End Date, making it a flexible tool for any reporting period. The dashboard is fully linked to raw transactional data, so changing the date filter instantly refreshes every KPI, table, and chart.",
        tools: ["Excel", "Sales Analysis", "Data Visualization"],
        highlights: [
            {
                title: "Dynamic Title & Date Range",
                text: "Header automatically reflects the selected reporting period."
            },
            {
                title: "Sales Flow KPIs",
                text: "Live Total Sales, Total Orders, Total Quantity, and Average Order Value."
            },
            {
                title: "Branch Performance",
                text: "Compare Sharkia, Monofya, Giza, Cairo, and more using sales and AOV metrics."
            },
            {
                title: "Salesman Contribution",
                text: "Measure each salesman's impact on total sales and orders."
            },
            {
                title: "Branch Comparison Chart",
                text: "Visual chart highlighting top and bottom performing branches."
            }
        ],
        work: [
    ...
    analysis: {
        ...
        subtitle: "Interactive financial dashboard for revenue, expenses, profit trends, and departmental performance.",
        type: "Financial Dashboard",
        tags: ["Excel Dashboards", "KPI Dashboards", "Microsoft Excel"],
        description: "As part of my Data Analysis learning journey, I built this interactive Financial Performance Dashboard to visualize a company's overall financial and operational performance. It focuses on clear UX and easy navigation across Overview, Revenue, Expenses, and Profit pages using a sidebar menu.",
        tools: ["Excel Dashboards", "KPI Dashboards", "Microsoft Excel"],
        highlights: [
            {
                title: "Top Financial KPIs",
                text: "Revenue, Expenses, Profit, Max Revenue, and Max Expenses at a glance."
            },
            {
                title: "Category Breakdowns",
                text: "Donut charts for Expenses by Category and Revenue by Category."
            },
            {
                title: "Profit by Month",
                text: "Line chart tracking monthly profit trends and fluctuations."
            },
            {
                title: "Department Profitability",
                text: "Compare IT, Marketing, HR, Finance, and Operations."
            },
            {
                title: "Interactive Slicers",
                text: "Filter by Year, Quarter, Month, Department, and Payment Method."
            }
        ],
        work: [
...
function renderProjectMeta(project) {
    if (!projectDetailsMeta) {
        return;
    }

    const imageCount = project.images ? project.images.length : 0;
    const toolCount = project.tools ? project.tools.length : 0;
    ...
    projectDetailsMeta.innerHTML = `
        <div class="project-meta-item">
            ...
            <span><strong>${project.type || "Case Study"}</strong></span>
        </div>
        <div class="project-meta-item">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 4h7v7H4V4Zm9 0h7v7h-7V4ZM4 13h7v7H4v-7Zm9 3h7v4h-7v-4Z"/></svg>
        ...
        <div class="project-meta-item">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5h16a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Zm1 2v8.2l3.2-3.2 2.3 2.3 3.4-3.4L19 15.5V7H5Z"/></svg>
            <span><strong>${imageCount}</strong> screenshots</span>
        </div>
    `;
}
...
function renderTools(tools) {
    ...
    tools.forEach(function (toolName) {
        ...
        tool.className = "project-tool";
        tool.innerHTML = toolIconSvg + "<span></span>";
        tool.querySelector("span").textContent = toolName;
        projectDetailsTools.appendChild(tool);
    });
}

function renderHighlights(highlights) {
    if (!projectDetailsHighlights) {
        return;
    }
    ...
    (highlights || []).forEach(function (item) {
        ...
        card.className = "project-highlight";
        card.innerHTML = `
            <div class="project-highlight-icon">${highlightIconSvg}</div>
            <h3></h3>
            <p></p>
        `;
        card.querySelector("h3").textContent = item.title;
        card.querySelector("p").textContent = item.text;
        projectDetailsHighlights.appendChild(card);
    });
}
...
function renderPointList(container, items) {
    ...
    items.forEach(function (itemText, index) {
        ...
        item.className = "project-point";
        item.innerHTML = `<span class="project-point-index">${index + 1}</span>`;
        item.appendChild(document.createTextNode(itemText));
        container.appendChild(item);
    });
}

function updateNextProjectButton(projectId) {
    if (!nextProjectBtn) {
        return;
    }
    ...
    nextProjectBtn.dataset.nextProject = nextId;
    nextProjectBtn.innerHTML = `
        Next: ${nextProject.title.split(" ").slice(0, 3).join(" ")}...
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9.5 6 15.5 12l-6 6-1.4-1.4 4.6-4.6-4.6-4.6L9.5 6Z"/></svg>
    `;
}
...
function populateProject(
    ...
    if (
        ...
        projectDetailsSubtitle.textContent =
            project.subtitle;

        projectDetailsSubtitle.style.display =
            "block";
    ...
    renderTagList(projectDetailsTags, project.tags);
    renderProjectMeta(project);
    renderTools(project.tools);
    renderHighlights(project.highlights);
    renderPointList(projectDetailsWork, project.work);
    renderPointList(projectDetailsInsights, project.insights);

    if (currentProjectId) {
        updateNextProjectButton(currentProjectId);
    }
...
function openProject(
    ...
    projectDetailsPage.setAttribute(
        "aria-hidden",
        "false"
    );
    ...
    images.forEach(
        function (
            ...
            screenshot.innerHTML = `
                <span class="project-screenshot-badge">0${index + 1}</span>
            ...
            screenshot.insertBefore(
                image,
                screenshot.querySelector(".project-screenshot-overlay")
            );


            projectScreenshotsGrid.appendChild(
                screenshot
            );
...
if (
    ...
    backToProjectsBottom.addEventListener(
        "click",
        function () {
...
if (nextProjectBtn) {
    nextProjectBtn.addEventListener("click", function () {
        const nextId = nextProjectBtn.dataset.nextProject;
...
function updateImageTransform() {
    ...
    projectLightboxImage.style.transform =
...
projectLightboxStage.addEventListener(
...
closeProjectLightbox.addEventListener(
